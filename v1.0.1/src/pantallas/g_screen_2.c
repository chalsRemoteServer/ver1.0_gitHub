...
//extern void ui_get_counters(uint32_t* below, uint32_t* above); // (ya existe del lado Arduino; si ya la migraste, perfecto)
...
static void cont_draw(void)
{
    ...
    // 1) título (relativo al content area)
    const uint16_t title_x = (uint16_t)(L->content_x0 + 30);
    const uint16_t title_y = (uint16_t)(L->content_y0 + 30);

    EVE_color_mask(1,1,1,1);
    EVE_color_a(255);
    EVE_color_rgb(BLACK);
    EVE_cmd_text_bold(title_x, title_y, 30, EVE_OPT_FLAT, "Contadores");

    // 2) parámetros (equivalentes Arduino, pero en px fijos)
    const uint16_t TITLE_ROWS_DY = 70;  // título -> primera fila
    const uint16_t TITLE_CARD_DY = 40;  // título -> top tarjetas
    //const uint16_t CARD_HDR_DY   = 12;
...
//       EVE_cmd_text(L->user_x,  L->user_y,  24, EVE_OPT_FLAT, user);    //


    // 4) tarjetas
    //auto no; // (NO usar auto en C) -> usamos tu draw_card() existente
