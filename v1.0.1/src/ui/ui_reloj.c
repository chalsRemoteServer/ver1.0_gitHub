...
// Ajusta includes según tu proyecto EVE
#include "EVE.h"
#include "colores.h"   // si ahí tienes BLACK, MEDIUM_GRAY, etc.
...
void ui_reloj_draw(const UiReloj *r, const UiMainLayout *L, const char *user)
{
    ...
    //EVE_cmd_text(L->clock_x, L->clock_y, L->font, EVE_OPT_FLAT, buf);    // Reloj
    //EVE_cmd_text(L->user_x,  L->user_y,  L->font, EVE_OPT_FLAT, user);    // Usuario
