/*
 * view_opciones_perifericos_autotest.c
 *
...
static void opciones_perifericos_autotest_draw(void)
{
    ...
    EVE_color_a(255);
    EVE_color_rgb(BLACK);
    EVE_cmd_text_bold(title_x, title_y, PRODUCT_FONT_SIZE, EVE_OPT_FLAT, "Prueba automatica");

    // ---- Texto placeholder ----
    EVE_color_rgb(BLACK);
    EVE_cmd_text(title_x + 150, title_y + 150, PRODUCT_FONT_SIZE, EVE_OPT_FLAT, "Relacionado a patrones");
...
static void opciones_perifericos_autotest_on_released(uint16_t id)
{
    switch (id)
    {
...
const UiView VIEW_OPCIONES_PERIFERICOS_AUTOTEST =
{
    .id = 58,
    .on_enter = 0,
    .on_exit = 0,
    .update = 0,
    .draw = opciones_perifericos_autotest_draw,
    .on_released = opciones_perifericos_autotest_on_released
};
