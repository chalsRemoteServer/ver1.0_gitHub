...
static void reg_draw(void)
{
	...
	//const uint16_t title_x = (uint16_t)(L->content_x0 + 60);
	const uint16_t title_y = (uint16_t)(L->content_y0 + 50);
	//const uint16_t table_x = (uint16_t)(L->content_x0 - 60);
	const uint16_t table_x = 20;

	const uint16_t table_y = (uint16_t)(title_y + 20);
	const uint16_t table_w = (uint16_t)(L->content_x1 - 50 - table_x); // ancho real desde table_x
	...
	// Título
	EVE_color_rgb(BLACK);
	EVE_cmd_text_bold(20, L->content_y0 + 10, 30, EVE_OPT_FLAT, "REGISTRO");
	...
	for (uint8_t i = 0; i < visibleRows; i++)
	{
		...
		// Fondo alternado
		if ((i & 1u) == 0u) EVE_color_rgb(MEDIUM_GRAY);
		...
		// Línea 1 (4 columnas)
		EVE_color_rgb(0x404040);
		EVE_cmd_text_bold(col0, (uint16_t)(y + pad_y), font_header, 0, s_rows[idx].id);
		EVE_cmd_text_bold(col1, (uint16_t)(y + pad_y), font_header, 0, s_rows[idx].fecha);
		EVE_cmd_text_bold(col2, (uint16_t)(y + pad_y), font_header, 0, s_rows[idx].hora);
		EVE_cmd_text_bold(col3, (uint16_t)(y + pad_y), font_header, 0, s_rows[idx].usuario);

		// Línea 2 (detalle)
		EVE_color_rgb(0x606060);
		EVE_cmd_text((int16_t)col0, (int16_t)(y + pad_y + 34), font_detail, 0, s_rows[idx].detalle);
	}

    const uint16_t x = (uint16_t)(L->content_x1 + 10);   // pegado a derecha del content
    uint16_t y = (uint16_t)(L->content_y0);
    const uint16_t w = 84;
    const uint16_t h = 84;
    ...
    ui_subm_draw_side_buttons(xR0, yR0, w, h, 10, side, 4);

//    // CRUZ ROJA (BACK)
//    EVE_color_mask(1, 1, 1, 1);
//    EVE_color_a(255);
//    EVE_tag(UI_VID_REG_BACK);
//    ui_rect_register(UI_VID_REG_BACK, x, y, w, h);
//    EVE_cmd_fgcolor(0x9B6200);
//    EVE_color_rgb(RED);
//    EVE_cmd_button(x, y, w, h, 24, 256, "X");
//
//    y += h + gap;
//
//    // CONFIG
//    EVE_color_mask(1, 1, 1, 1);
//    EVE_color_a(255);
//    EVE_tag(UI_VID_REG_CONFIG);
//    ui_rect_register(UI_VID_REG_CONFIG, x, y, w, h);
//    EVE_cmd_fgcolor(0x9B6200);
//    EVE_color_rgb(WHITE);
//    EVE_cmd_button(x, y, w, h, 24, 256, "CFG");
//    y += h + gap;
//
//    // UP
//    EVE_color_mask(1, 1, 1, 1);
//    EVE_color_a(255);
//    EVE_tag(UI_VID_REG_UP);
//    ui_rect_register(UI_VID_REG_UP, x, y, w, h);
//
//    EVE_cmd_fgcolor(0x9B6200);
//    EVE_color_rgb(WHITE);
//    EVE_cmd_button(x, y, w, h, 24, 256, "^");
//    y += h + gap;
//
//    // DOWN
//    EVE_color_mask(1, 1, 1, 1);
//    EVE_color_a(255);
//    EVE_tag(UI_VID_REG_DOWN);
//    ui_rect_register(UI_VID_REG_DOWN, x, y, w, h);
//    EVE_cmd_fgcolor(0x9B6200);
//    EVE_color_rgb(WHITE);
//    EVE_cmd_button(x, y, w, h, 24, 256, "v");
//
//    // Siempre reset tag al final del bloque local (higiene)
//    EVE_tag(0);
}
