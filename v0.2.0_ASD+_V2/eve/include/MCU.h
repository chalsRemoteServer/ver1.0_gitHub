...
#define GPIO_SPIM_CLK	27 //pin 18, conn-1  tarjeta MM900EV-LITE
#define GPIO_SPIM_MISO	30 //pin 19, conn-1  tarjeta MM900EV-LITE
#define GPIO_SPIM_MOSI	29 //pin 20, conn-1  tarjeta MM900EV-LITE
...
#elif SPI_CHANNEL == 3
#   define SPI_SS_GPIO (35) //pin 17, conn-1  tarjeta MM900EV-LITE
...
//uint8_t MCU_SPImemRead8(uint32_t ft_address);
uint16_t MCU_SPImemRead16(uint32_t const ft_address);
uint32_t MCU_SPImemRead32(uint32_t ft_address);
...
void spi_transmit_burst(uint32_t var);
void spi_transmit_32(uint32_t var);
