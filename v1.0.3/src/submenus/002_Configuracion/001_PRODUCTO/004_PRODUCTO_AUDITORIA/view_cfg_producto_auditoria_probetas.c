/*
 * view_cfg_producto_auditoria_probetas.c
 *
...
static void opciones_producto_auditoria_probetas_draw(void)
{
    ...
    EVE_color_a(255);
    EVE_color_rgb(BLACK);
    EVE_cmd_text_bold(title_x, title_y, PRODUCT_FONT_SIZE, EVE_OPT_FLAT, "Patrones de prueba");
...
static void opciones_producto_auditoria_probetas_on_released(uint16_t id)
{
    switch (id)
    {
...
const UiView VIEW_OPCIONES_PRODUCTO_AUDITORIA_PROBETAS =
{
    .id = 16,
    .on_enter = 0,
    .on_exit = 0,
    .update = 0,
    .draw = opciones_producto_auditoria_probetas_draw,
    .on_released = opciones_producto_auditoria_probetas_on_released
};
